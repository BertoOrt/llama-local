...
app.controller('home', ['$scope', '$http', '$location', 'AuthUser', 'ipCookie', function ($scope, $http, $location, AuthUser, ipCookie) {
  ...
  $scope.sendMail = function () {
    ...
    $http.post('//localhost:3000/mail', data).success(function (response) {
...
app.controller('search', ['$scope', 'AuthUser', '$http', '$location', function ($scope, AuthUser, $http, $location) {
  ...
  $http.get('//localhost:3000/users').success(function (response) {
    console.log(response);
...
app.controller('user', ['$scope', '$http', 'ipCookie', '$location', 'AuthUser', function ($scope, $http, ipCookie, $location, AuthUser) {
  ...
  $http.post('//localhost:3000/user/info', { id: $scope.user.id, url: $scope.url }).success(function (response, stat) {
    if (response.status == "ok") {
  ...
  $scope.share = function () {
    ...
    $http.post('//localhost:3000/user/share', { id: $scope.user.facebookId, token: $scope.user.facebookToken, url: $location.path().substring(1) }).success(function (response) {
      console.log(response);
      if (response.status == "ok") {
